...
##        (overlay_set_color, reg1, 0),
  ...
  ("budget_report", 0, mesh_load_window, [
      (ti_on_presentation_load, [
          ...
          (store_mul, ":num_centers_needed_for_efficiency_loss", ":difficulty", 2), #Hardest: 2, Easiest: 10
          (store_mul, ":tax_efficiency_loss_ratio_per_center", "$g_difficulty", 2), #Easiest: 0, Hardest: 8

          (try_begin),
            (call_script, "script_is_king", "$players_kingdom", "trp_player"),
            (gt, reg0, 0), #Is player king/queen?
            (val_add, ":num_centers_needed_for_efficiency_loss", 5), #Add 5 more slot.
            (call_script, "script_get_lord_count", "$players_kingdom", 1),
            (gt, reg0, 0), #Does kingdom has lords?
            (val_mul, reg0, 2), #Add 2 slots per each lord.
            (val_add, ":num_centers_needed_for_efficiency_loss", reg0),
          (try_end),
          #
          ## UID: 148 - End
	    ...
	    (try_end),
            ...
            #
            #(party_slot_eq, ":center_no", slot_town_lord, "trp_player"),
            (call_script, "script_is_lord_of_party", "trp_player", ":center_no", 1, 0, 1), #Is lord of party, or spouse is lady/lord of party, or is player leader of the faction?
            (ge, reg8, 1),
            #
            ## UID: 148 - End
            (val_add, ":num_lines", 1),
          ...
          (try_begin),
            ...
            ## UID: 148 - Begin
            #
            (call_script, "script_is_king", "$players_kingdom", "trp_player"), #If (s)he has the faction but (s)he is not the leader of the faction. (I added that line because in DN system, you can leader npc kingdoms.)
            (le, reg0, 0),
            #
            ## UID: 148 - End
          ...
          (try_for_parties, ":party_no"),
            ...
            (else_try),
              ...
              (call_script, "script_is_king", ":center_faction", "trp_player"),
              (ge, reg0, 1),
              #
              ## UID: 148 - End
              (assign, ":garrison_troop", 1),
          ...
          (try_for_range, ":center_no", centers_begin, centers_end),
            ...
            #
            #(party_slot_eq, ":center_no", slot_town_lord, "trp_player"),
            (call_script, "script_is_lord_of_party", "trp_player", ":center_no", 1, 0, 1),
            (gt, reg8, 0),
            #
            ## UID: 148 - End
            ...
            #
            (try_begin),
              (call_script, "script_is_lord_of_party", "trp_player", ":center_no", 1, 0, 0),
              (le, reg8, 0), #Is player or spouse not lord of the party but leader of the kingdom?
              (val_mul, ":accumulated_total", 25), #Get 25% of the earnings...
              (val_div, ":accumulated_total", 100),
            (try_end),
            #
            ## UID: 148 - End
          ...
          (store_mul, ":sale_party", ":cur_leadership", 2), #2% sale for each leadership level.
          (store_mul, ":sale_garrison", ":cur_leadership", 6), #6% sale for each leadership level for garrison units.
          #
          ## UID: 148 - End
          (try_for_parties, ":party_no"),
            (assign, ":garrison_troop", 0),
            ...
            (else_try),
              ...
              ## UID: 148 - Begin
              #
              # We don't need to check fac_player_supporters_faction any more because player can leader the npc kingdoms too...
              #(eq, ":center_faction", "fac_player_supporters_faction"),
              #(faction_slot_eq, "fac_player_supporters_faction", slot_faction_leader, "trp_player"),
              (call_script, "script_is_king", ":center_faction", "trp_player"),
              (ge, reg0, 1),
            ...
            (try_begin),
              ...
              #(try_begin),
              #  (eq, ":garrison_troop", 1),
              #  (store_mul, ":perc", ":total_wage", ":sale_garrison"),
              #  (val_div, ":perc", 100),
              #  (val_sub, ":total_wage", ":perc"),
              #(else_try),
              #  (store_mul, ":perc", ":total_wage", ":sale_party"),
              #  (val_div, ":perc", 100),
              #  (val_sub, ":total_wage", ":perc"),
              #(try_end),
              (eq, ":garrison_troop", 1),
...
##          (val_add, ":num_pages", 3),
  ...
  ("mcc_character_creation", 0, 0, [
      (ti_on_presentation_load, [
          ...
          ## Modder login
          # We don't need to set pos for this, because we will use it on the left-bottom corner.
          (create_check_box_overlay, "$g_presentation_obj_modder", "mesh_checkbox_off", "mesh_checkbox_on"),
          (overlay_set_val, "$g_presentation_obj_modder", "$g_modder"),
          ...
          (create_combo_button_overlay, "$g_presentation_obj_religion", tf_center_justify),
          (position_set_x, pos1, 175),
          (position_set_y, pos1, 560),
          (overlay_set_position, "$g_presentation_obj_religion", pos1),
          (position_set_x, pos1, 800),
          (position_set_y, pos1, 800),
          (overlay_set_size, "$g_presentation_obj_religion", pos1),
          (overlay_add_item, "$g_presentation_obj_religion", "@Judaist"),
          (overlay_add_item, "$g_presentation_obj_religion", "@Sikhist"),
          (overlay_add_item, "$g_presentation_obj_religion", "@Nors"),
          (overlay_add_item, "$g_presentation_obj_religion", "@Tengrist"),
          (overlay_add_item, "$g_presentation_obj_religion", "@Atheist"),
          (overlay_add_item, "$g_presentation_obj_religion", "@Buddhist"),
          (overlay_add_item, "$g_presentation_obj_religion", "@Hindu"),
          (overlay_add_item, "$g_presentation_obj_religion", "@Muslim"),
          (overlay_add_item, "$g_presentation_obj_religion", "@Christian"),
          (store_sub, ":val", 8, "$g_religion"),
          (overlay_set_val, "$g_presentation_obj_religion", ":val"),
          #
          ## Religion - End
          ...
          (overlay_add_item, "$g_presentation_obj_region", "@Kielian Sultanate"),
          (overlay_add_item, "$g_presentation_obj_region", "@Kingdom of Umalelith"),
          ...
          ## Hardness - Begin
          #
          (create_text_overlay, reg1, "@My life as hard as a...", tf_center_justify),
          (position_set_x, pos1, 1000),
          (position_set_y, pos1, 1000),
          (overlay_set_size, reg1, pos1),
          (position_set_x, pos1, 150),
          ...
          (create_combo_button_overlay, "$g_presentation_obj_hardness", tf_center_justify),
          (position_set_x, pos1, 175),
          (position_set_y, pos1, 200),
          (overlay_set_position, "$g_presentation_obj_hardness", pos1),
          (position_set_x, pos1, 800),
          (position_set_y, pos1, 800),
          (overlay_set_size, "$g_presentation_obj_hardness", pos1),
          (overlay_add_item, "$g_presentation_obj_hardness", "@Casual"),
          (overlay_add_item, "$g_presentation_obj_hardness", "@Easy"),
          (overlay_add_item, "$g_presentation_obj_hardness", "@Moderate"),
          (overlay_add_item, "$g_presentation_obj_hardness", "@Hard"),
          (overlay_add_item, "$g_presentation_obj_hardness", "@Realistic"),
          (overlay_set_val, "$g_presentation_obj_hardness", "$g_difficulty"),
          #
          ## Hardness - End
      ...
      (ti_on_presentation_event_state_change, [
          ...
          (else_try), ####### MODDER LOGIN #######
            (eq, ":object", "$g_presentation_obj_modder"),
            (assign, "$g_modder", ":value"),
          ...
          (else_try), ####### GENDER MENU #######
            ...
            (troop_set_type, "trp_player", ":value"),
            (start_presentation, "prsnt_mcc_character_creation"),
            (assign, "$g_presentation_obj_gender", ":value"),
          (else_try), ####### HARDNESS MENU #######
            (eq, ":object", "$g_presentation_obj_hardness"),
            (assign, "$g_difficulty", ":value"),
            (start_presentation, "prsnt_mcc_character_creation"),
            (assign, "$g_presentation_obj_hardness", ":value"),
          (else_try), ####### RELIGION MENU #######
            (eq, ":object", "$g_presentation_obj_religion"),
            (store_sub, ":val", 8, ":value"),
            (assign, "$g_religion", ":val"),
            (start_presentation, "prsnt_mcc_character_creation"),
            (assign, "$g_presentation_obj_religion", ":value"),
  ...
  ("test", 0, 0, [
      (ti_on_presentation_load, [
          (display_message, "@Presentation Loaded!"),

          (presentation_set_duration, 999999),
          (set_fixed_point_multiplier, 1000),

          (create_mesh_overlay, reg1, "mesh_party_window"),
          (position_set_x, pos1, 0),
          (position_set_y, pos1, 0),
          (overlay_set_position, reg1, pos1),
      ...
      (ti_on_presentation_event_state_change, [
          ...
          (store_trigger_param_2, reg2),
          (display_message, "@Event State Change: {reg1} - {reg2}"),
        ]),

      (ti_on_presentation_mouse_enter_leave, [
          (store_trigger_param_1, reg1),
          (store_trigger_param_2, reg2),
          (display_message, "@Mouse Enter-Leave: {reg1} - {reg2}"),
        ]),

      (ti_on_presentation_mouse_press, [
          (store_trigger_param_1, reg1),
          (store_trigger_param_2, reg2),
          (display_message, "@Mouse Press: {reg1} - {reg2}"),
          (try_begin),
            (eq, reg2, 2),
            (presentation_set_duration, 0),
          (try_end),
